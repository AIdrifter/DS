...
void doubleNegate(const void *pObj, void *pRet){
    *(double*)pRet = (-1)*(*(double*)pObj);
}
void shortSum(const void *a, const void *b, void *pRet){
    *(short*)pRet = *(short*)a + *(short*)b;
}
void shortNegate(const void *pObj, void *pRet){
    *(short*)pRet = (-1)*(*(short*)pObj);
}
        ...
        if(p == NULL){
            ...
            if(p == NULL)
                ...
                if(type == __DS__DOUBLE__)
                    ret = rsqInit(p, sizeof(double), cap, doubleSum, doubleNegate, &double_zero);
                else
                    ret = rsqInit(p, sizeof(short), cap, shortSum, shortNegate, &short_zero);
            }
